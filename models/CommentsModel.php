...
class CommentsModel extends Model
{
	 ...
	 * @return Collection|CommentsModel[]|CommentsModel|null A collection of models or null if there are no comments
	 */
        public static function findPublishedBySourceAndParentAndProductId($strSource, $intParent, $itemId, $blnDesc=false, $intLimit=0, $intOffset=0, array $arrOptions=array())
	{
		$t = static::$strTable;
		$arrColumns = array("$t.source=? AND $t.parent=? AND $t.idprodukt=?");
		...
		return static::findBy($arrColumns, array($strSource, (int) $intParent, $itemId), $arrOptions);
	}
	 ...
	 * @param string  $strSource  The source element
	 * @param integer $intParent  The parent ID
         * @param integer $itemId     The eCommerce Item Id
	 * @param array   $arrOptions An optional options array
	 *
	 * @return integer The number of comments
	 */
	public static function countPublishedBySourceAndParentAndProductId($strSource, $intParent, $itemId, array $arrOptions=array())
	{
		$t = static::$strTable;
		$arrColumns = array("$t.source=? AND $t.parent=? AND $t.idprodukt=?");
		...
		return static::countBy($arrColumns, array($strSource, (int) $intParent, $itemId));
	}
}
